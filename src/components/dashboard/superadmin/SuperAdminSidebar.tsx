...
// ponytail: same sidebar pattern, routes to /dashboard/superadmin
export function SuperAdminSidebar() {
  ...
  const isDashboardActive = pathname === "/dashboard/superadmin";
  const isManajemenActive = pathname.startsWith("/dashboard/superadmin/manajemen-pengguna");
  const isProfilActive = pathname === "/dashboard/superadmin/profil";
  ...
  return (
    <aside className="w-65 shrink-0 bg-brand-dark flex flex-col p-0 sticky top-0 h-screen overflow-y-auto max-[600px]:hidden">
      ...
      <nav className="pt-4.5 px-3 pb-0 flex-1">
        ...
        <Link href="/dashboard/superadmin" className={linkClass(isDashboardActive)}>
        ...
        <Link href="/dashboard/superadmin/manajemen-pengguna" className={linkClass(isManajemenActive)}>
          <svg viewBox="0 0 24 24" fill="none" className={iconClass(isManajemenActive)}>
        ...
        <Link href="/dashboard/superadmin/profil" className={linkClass(isProfilActive)}>
      ...
      <div className="p-4 px-3 border-t border-white/8">
        <div className="flex items-center gap-3 py-2.5 px-3 rounded-2.5 mb-1.5 bg-white/5">
          <div className="w-9 h-9 rounded-full bg-brand flex items-center justify-center text-[14px] shrink-0 overflow-hidden">👑</div>
          <div className="min-w-0">
            ...
            <div className="text-2.75 text-white/50 whitespace-nowrap overflow-hidden text-ellipsis">Administrator Utama</div>
          </div>
        </div>
